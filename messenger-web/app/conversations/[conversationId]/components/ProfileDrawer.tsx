...
export default function ProfileDrawer({ isOpen, onClose, data }: ProfileDrawerProps) {
  ...
  const joinedDate = useMemo(() => {
    return format(new Date(otherUser.createdAt), 'PP'); // Ex: Apr 29, 1453
  }, [otherUser.createdAt]);
  ...
  const statusText = useMemo(() => {
    ...
    return isActive ? 'Active now' : `Last seen ${joinedDate}`;
  }, [data.isGroup, data.users.length, isActive, joinedDate]);
